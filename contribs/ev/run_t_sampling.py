...
def update_Q(Q: pd.DataFrame, chosen_links, score, threshold):
    # Set 'link_id' as the index for faster lookups
    Q = Q.set_index("link_id")

    for link in chosen_links:
        if link in Q.index:  # Check if the link exists in the index
            if score > Q.loc[link, "high_score"]:
                Q.loc[link, "high_score"] = score
            if score > threshold:  # Define a success condition (threshold is defined as the upper 25th percentile)
                Q.loc[link, "success_count"] += 1
            else:
                Q.loc[link, "failure_count"] += 1
        else:
...
def main(args):
    ...
    if os.path.exists(args.q_path):
        ...
        Q = pd.DataFrame({
            "link_id": link_ids,
            "success_count": np.ones(len(link_ids)),  # Start with 1 to avoid divide-by-zero
            "failure_count": np.ones(len(link_ids)),  # Start with 1
            "high_score": np.zeros(len(link_ids)),
        })
    ...
    # run num_runs times using the chosen algorithm
    for i in range(1, num_runs + 1):
        print_run_info(i, num_runs)

        if args.algorithm == "tsampling":
            chosen_links = thompson_sampling(args.num_chargers, Q)
        ...
        q_scores = Q[Q["high_score"] != 0]["high_score"]
        threshold = q_scores.quantile(0.40)
        # This is a little flawed on the first run, because it automatically marks the first chargers chosen as failures if we don't do any exploration first

        # Update Q values
        Q = update_Q(Q, chosen_links, average_score, threshold)
        save_Q(Q, args.q_path)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--algorithm",help="Algorithm to use for optimization",choices=["montecarlo","egreedy","tsampling"],
                            default="tsampling",type=str)
