...
def create_sensor_osm(flows_path: str, locations_path: str):
    ...
    for sensor_id, (lat, lon) in locations.items():
        node = ET.SubElement(osm_root, "node", id=str(node_id), lat=str(lat), lon=str(lon), visible="true")
        ...
        if sensor_id in flows:
            for hour, flow in enumerate(flows[sensor_id]):
                ET.SubElement(node, "tag", k=f"flow_{hour:02d}", v=str(flow))  # Store flow as a string
